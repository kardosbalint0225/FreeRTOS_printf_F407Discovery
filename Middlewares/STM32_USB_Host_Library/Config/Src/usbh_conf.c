...
void HAL_HCD_MspDeInit(HCD_HandleTypeDef* hcdHandle)
{
	if(hcdHandle->Instance==USB_OTG_FS)
	{
		...
		PA12     ------> USB_OTG_FS_DP
		*/
		HAL_GPIO_DeInit(GPIOA, GPIO_PIN_9|OTG_FS_ID_Pin|OTG_FS_DM_Pin|OTG_FS_DP_Pin);
...
USBH_StatusTypeDef USBH_LL_Init(USBH_HandleTypeDef *phost)
{
	...
	if (phost->id == HOST_FS)
	{
		...
		ret = HAL_HCD_Init(&hhcd_USB_OTG_FS);
...
USBH_StatusTypeDef USBH_LL_DeInit(USBH_HandleTypeDef *phost)
{
	...
	assert_param(USBH_OK == usb_status);
									...
									uint16_t mps)
{
	...
	hal_status = HAL_HCD_HC_Init(phost->pData,
								 ...
								 ep_type,
								 mps);
	assert_param(hal_status == HAL_OK);
